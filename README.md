...
```
Frontend:  React 19 + TypeScript + Vite + Tailwind CSS 4 + Motion   →  :5173
...
│   ├── src/
│   │   ├── components/
│   │   │   ├── Board/            # SVG board: points, checkers, bar, bear-off
│   │   │   ├── Dice/             # 3D rolling cube animation + dice display
│   │   │   ├── GameScreen/       # Main game container with overlays
│   │   │   ├── HomeScreen/       # Room creation, match settings, active rooms
│   │   │   ├── WaitingRoom/      # Room code, copy, wait for opponent
│   │   │   ├── AuthScreen/       # Login / Register
│   │   │   ├── Controls/         # Double, End Turn buttons (Roll removed — handled by overlay)
│   │   │   ├── MatchSettings/    # Pre-game config: color, target points
│   │   │   ├── GameResultOverlay/ # Post-game result + match score + auto-advance
│   │   │   ├── DoublingCube/     # Cube value + owner display
│   │   │   ├── OpponentBar/      # Opponent info + WINNER badge
...
- **3D dice**: Six-face CSS cube with `preserve-3d`, `rotateX/Y/Z` animation, `backfaceVisibility: hidden`.
...
- **CSS Modules + Tailwind**: CSS Modules for component-scoped styles, Tailwind for layout/utilities.
